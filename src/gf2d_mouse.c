...
Edge3D gf2d_mouse_get_cast_ray()
{
    ...
    res = gf3d_vgraphics_get_view_extent_as_vector2d(); 
    ...
    vector3d_scale(rightNear, right, -1 * w.x * (res.x*0.125f));
    vector3d_scale(upNear, up, -1 * w.y * (res.y*0.125f));
    a.x = position.x + rightNear.x + upNear.x;
    a.y = position.y + rightNear.y + upNear.y;
    a.z = position.z + rightNear.z + upNear.z;
    vector3d_scale(forward, forward, 2048);
    vector3d_scale(right, right, -2048 * w.x);
    vector3d_scale(up, up, -2048 * w.y);
...
Vector3D gf2d_mouse_screen_to_world() {
    Vector2D mouse = gf2d_mouse_get_position();

    Vector2D res = gf3d_vgraphics_get_view_extent_as_vector2d();

    Vector3D pt;
    pt.x = -(mouse.x/res.x * 256 - 128);
    pt.y = 0;
    pt.z = -(mouse.y/res.y * 256 - 128);
    return pt;
}
