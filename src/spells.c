...
void S_statmap_init() {
	...
	SpellInfo* freeze = (SpellInfo*)malloc(sizeof(SpellInfo));
	SpellInfo* heal = (SpellInfo*)malloc(sizeof(SpellInfo));
	SpellInfo* sludge = (SpellInfo*)malloc(sizeof(SpellInfo));
	SpellInfo* smite = (SpellInfo*)malloc(sizeof(SpellInfo));
	SpellInfo* pyro = (SpellInfo*)malloc(sizeof(SpellInfo));
...
void S_try_buy(Vector3D position, char* name) {
	...
	if (saveData->elixir < info->cost) {
		slog("Come back when you're a little, mmm, richer! %i",info->cost);
		return;
	}
...
void S_draw(Entity* self) {
	SpellInfo* info = (SpellInfo*)self->customData;
	gf3d_draw_circle(gfc_circle(0,0,info->radius), self->position, vector3d(0, 0, 0), vector3d(1, 1, 1), gfc_color(info->color.r,info->color.g,info->color.b, (SDL_GetTicks() - self->fireTimer/(float)info->duration)));
}